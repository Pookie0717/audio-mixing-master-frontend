...
const Blog = () => {
    ...
    const renderKeywords = (keywords) => {
        ...
        // For blog listing, show only first 2-3 keywords to save space
        ...
        return (
            <div className="flex flex-wrap gap-1 mt-3">
                {displayKeywords.map((keyword, index) => (
                    <span 
                        key={index}
                        className="bg-[#1a1a1a] text-[#4CC800] px-2 py-1 rounded-full text-xs font-medium border border-[#4CC800] border-opacity-30"
                    >
                        {keyword}
                    </span>
                ))}
    ...
    const selectedCategoryName = selectedCategory === 'all' 
        ? 'All Categories' 
        : categories.find(cat => cat.id === selectedCategory)?.name || 'Blog';

    return (
        <>
            <SEO 
                title={`${selectedCategoryName} - Audio Blog`}
                description="Discover the latest insights, tips, and techniques in audio mixing and mastering. From beginner guides to advanced production secrets."
                keywords="audio mixing, mastering, music production, recording tips, audio engineering, studio techniques"
                url="/blog"
                type="website"
            />
                                ...
                                >
                                    <div className="grid lg:grid-cols-2 gap-8 items-center">
                                        <div>
                                            ...
                                            {/* Keywords for featured post */}
                                            {renderKeywords(featuredPost.keywords)}
                                        </div>
                                        <div>
                                            <img 
